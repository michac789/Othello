...
def simulate(N, level_black, level_white, show):
    print(f"Simulating AI Level {level_black} VS AI Level {level_white} for {N} times...")
    win_black, win_white = 0, 0
    for i in range(N):
        if show: print(f"Playing game {i+1} out of {N}...")
        ot = Othello(8, 8)
        ...
        if show: print(f"Winner: {ot.check_victory()}")
